                ...
                <div class="content container-fluid">
                    ...
                    <div class="page-header">
                        <div class="row align-items-center">
                            <div class="col">
                                <h3 class="page-title">{{ trans('main.Course Items') }}</h3>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ trans('main.Dashboard') }}</a></li>
                                    <li class="breadcrumb-item active">{{ trans('main.Course Items') }}</li>
                                </ul>
                            </div>
                            <div class="col-auto">
                    ...
                    <div class="card filter-card" id="filter_inputs" @if($name || $from_date || $to_date) style="display:block" @endif>
                        <div class="card-body pb-0">
                            <form action="{{ route('course-item.index') }}" method="get">
                    ...
                    <div class="row">
                                        ...
                                        <table id="example1" class="table table-stripped">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center">{{ trans('main.Author') }}</th>
                                            ...
                                            <tbody>
                                                @if($data->count() > 0)
                                                    ...
                                                    @foreach ($data as $item)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center notPrint">
                                                                @if($item->photo)
                                                                    ...
                                                                    <br>
                                                                    <a class="btn btn-outline-success btn-sm" href="{{ route('show_file',['course-item',$item->photo]) }}" role="button" target="_blank"><i class="fas fa-eye"></i></a>
                                                                    <a class="btn btn-outline-info btn-sm" href="{{ route('download_file',['course-item',$item->photo]) }}" role="button" target="_blank"><i class="fas fa-download"></i></a>
                                                                @endif
                                                            </td>
                                                        ...
                                                        @include('dashboard.course-item.deleteModal')
                                                        @include('dashboard.course-item.deleteSelectedModal')
                                                    @endforeach
                                                @else
                                                    <tr>
                        ...
                        @include('dashboard.course-item.addModal')
                        @include('dashboard.course-item.editModal')		
                    </div>	
                </div>
            </div>
            <!-- /Page Wrapper -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '#storeBtn', function (e) {
                ...
                formData.append('author_ar', $('.author_ar').val());
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status == false) {
                            ...
                            $('#update_author_ar').val(response.data.author_ar);
            ...
            $(document).on('click', '.updateBtn', function (e) {
                ...
                formData.append('author_ar', $('#update_author_ar').val());
            ...
            $(document).on('click', '.destroyBtn', function (e) {
                ...
                $.ajax({
                    type: "delete",
                    url: "/admin/course-item/destroy/" + id,
                    success: function (response) {
                        if (response.status == false) {
                            $('#delete_error_list').html("");
