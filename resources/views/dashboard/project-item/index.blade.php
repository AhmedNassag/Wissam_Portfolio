                ...
                <div class="content container-fluid">
                    ...
                    <div class="page-header">
                        <div class="row align-items-center">
                            <div class="col">
                                <h3 class="page-title">{{ trans('main.Project Items') }}</h3>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ trans('main.Dashboard') }}</a></li>
                                    <li class="breadcrumb-item active">{{ trans('main.Project Items') }}</li>
                                </ul>
                            </div>
                            <div class="col-auto">
                    ...
                    <div class="card filter-card" id="filter_inputs" @if($name || $from_date || $to_date) style="display:block" @endif>
                        <div class="card-body pb-0">
                            <form action="{{ route('project-item.index') }}" method="get">
                    ...
                    <div class="row">
                                                ...
                                                @if($data->count() > 0)
                                                    ...
                                                    @foreach ($data as $item)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center notPrint">
                                                                @if($item->photo)
                                                                    <img class="mb-1" src="{{ asset('attachments/project-item/'.$item->photo) }}" alt="{{ $item->photo }}" height="50" width="70">
                                                                    <br>
                                                                    <a class="btn btn-outline-success btn-sm" href="{{ route('show_file',['project-item',$item->photo]) }}" role="button" target="_blank"><i class="fas fa-eye"></i></a>
                                                                    <a class="btn btn-outline-info btn-sm" href="{{ route('download_file',['project-item',$item->photo]) }}" role="button" target="_blank"><i class="fas fa-download"></i></a>
                                                                @endif
                                                            </td>
                                                        ...
                                                        @include('dashboard.project-item.deleteModal')
                                                        @include('dashboard.project-item.deleteSelectedModal')
                                                    @endforeach
                                                @else
                                                    <tr>
                        ...
                        @include('dashboard.project-item.addModal')
                        @include('dashboard.project-item.editModal')		
                    </div>	
                </div>
            </div>
            <!-- /Page Wrapper -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.editBtn', function (e) {
                ...
                $.ajax({
                    type: "get",
                    url: "/admin/project-item/edit/" + id,
                    success: function (response) {
                        if (response.status == false) {
                            $('#edit_error_list').html("");
                            ...
                            $('#preview_image').attr('src', '/attachments/project-item/' + response.data.photo);
            ...
            $(document).on('click', '.destroyBtn', function (e) {
                ...
                $.ajax({
                    type: "delete",
                    url: "/admin/project-item/destroy/" + id,
                    success: function (response) {
                        if (response.status == false) {
                            $('#delete_error_list').html("");
