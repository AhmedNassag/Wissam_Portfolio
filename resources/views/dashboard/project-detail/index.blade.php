                ...
                <div class="content container-fluid">
                    ...
                    <div class="page-header">
                        <div class="row align-items-center">
                            <div class="col">
                                <h3 class="page-title">{{ trans('main.Project Details') }}</h3>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ trans('main.Dashboard') }}</a></li>
                                    <li class="breadcrumb-item active">{{ trans('main.Project Details') }}</li>
                                </ul>
                            </div>
                            <div class="col-auto">
                                @if($data->count() == 0)
                    ...
                    <div class="card filter-card" id="filter_inputs" @if($details || $from_date || $to_date) style="display:block" @endif>
                        <div class="card-body pb-0">
                            <form action="{{ route('project-detail.index') }}" method="get">
                    ...
                    <div class="row">
                                            ...
                                            <tbody>
                                                @if($data->count() > 0)
                                                    ...
                                                    @foreach ($data as $item)
                                                        ...
                                                        @include('dashboard.project-detail.deleteModal')
                                                        @include('dashboard.project-detail.deleteSelectedModal')
                                                    @endforeach
                                                @else
                                                    <tr>
                        ...
                        @include('dashboard.project-detail.addModal')
                        @include('dashboard.project-detail.editModal')		
                    </div>	
                </div>
            </div>
            <!-- /Page Wrapper -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '#storeBtn', function (e) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('project-detail.store') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    dataType: "json",
            ...
            $(document).on('click', '.editBtn', function (e) {
                ...
                $.ajax({
                    type: "get",
                    url: "/admin/project-detail/edit/" + id,
                    success: function (response) {
                        if (response.status == false) {
                            $('#edit_error_list').html("");
            ...
            $(document).on('click', '.updateBtn', function (e) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('project-detail.update') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    dataType: "json",
            ...
            $(document).on('click', '.destroyBtn', function (e) {
                ...
                $.ajax({
                    type: "delete",
                    url: "/admin/project-detail/destroy/" + id,
                    success: function (response) {
                        if (response.status == false) {
                            $('#delete_error_list').html("");
