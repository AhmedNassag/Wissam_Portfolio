                ...
                <div class="content container-fluid">
                    ...
                    <div class="page-header">
                        <div class="row align-items-center">
                            <div class="col">
                                <h3 class="page-title">{{ trans('main.Parteners') }}</h3>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ trans('main.Dashboard') }}</a></li>
                                    <li class="breadcrumb-item active">{{ trans('main.Parteners') }}</li>
                                </ul>
                            </div>
                            <div class="col-auto">
                    ...
                    <div class="card filter-card" id="filter_inputs" @if($name || $from_date || $to_date) style="display:block" @endif>
                        <div class="card-body pb-0">
                            <form action="{{ route('partener.index') }}" method="get">
                    ...
                    <div class="row">
                                                ...
                                                @if($data->count() > 0)
                                                    ...
                                                    @foreach ($data as $item)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center notPrint">
                                                                @if($item->photo)
                                                                    <img class="mb-1" src="{{ asset('attachments/partener/'.$item->photo) }}" alt="{{ $item->photo }}" height="50" width="70">
                                                                    <br>
                                                                    <a class="btn btn-outline-success btn-sm" href="{{ route('show_file',['partener',$item->photo]) }}" role="button" target="_blank"><i class="fas fa-eye"></i></a>
                                                                    <a class="btn btn-outline-info btn-sm" href="{{ route('download_file',['partener',$item->photo]) }}" role="button" target="_blank"><i class="fas fa-download"></i></a>
                                                                @endif
                                                            </td>
                                                        ...
                                                        @include('dashboard.partener.deleteModal')
                                                        @include('dashboard.partener.deleteSelectedModal')
                                                    @endforeach
                                                @else
                                                    <tr>
                        ...
                        @include('dashboard.partener.addModal')
                        @include('dashboard.partener.editModal')		
                    </div>	
                </div>
            </div>
            <!-- /Page Wrapper -->
    ...
    <script>
        $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status == false) {
                            ...
                            $('#update_url').val(response.data.url);
                            $('#preview_image').attr('src', '/attachments/partener/' + response.data.photo);
            ...
            $(document).on('click', '.updateBtn', function (e) {
                ...
                formData.append('url', $('#update_url').val());
