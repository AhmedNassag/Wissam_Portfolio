                ...
                <div class="content container-fluid">
                    ...
                    <div class="page-header">
                        <div class="row align-items-center">
                            <div class="col">
                                <h3 class="page-title">{{ trans('main.Who We Are Faqs') }}</h3>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ trans('main.Dashboard') }}</a></li>
                                    <li class="breadcrumb-item active">{{ trans('main.Who We Are Faqs') }}</li>
                                </ul>
                            </div>
                            <div class="col-auto">
                    ...
                    <div class="card filter-card" id="filter_inputs" @if($name || $from_date || $to_date) style="display:block" @endif>
                        <div class="card-body pb-0">
                            <form action="{{ route('who-we-are-faq.index') }}" method="get">
                                <div class="row filter-row">
                                    <div class="col-sm-6 col-md-3">
                                        <div class="form-group">
                                            <label>{{ trans('main.Question') }}</label>
                    ...
                    <div class="row">
                                        ...
                                        <table id="example1" class="table table-stripped">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center">{{ trans('main.Question') }}</th>
                                                    <th class="text-center">{{ trans('main.Answer') }}</th>
                                            ...
                                            <tbody>
                                                @if($data->count() > 0)
                                                    ...
                                                    @foreach ($data as $item)
                                                        ...
                                                        @include('dashboard.who-we-are-faq.deleteModal')
                                                        @include('dashboard.who-we-are-faq.deleteSelectedModal')
                                                    @endforeach
                                                @else
                                                    <tr>
                        ...
                        @include('dashboard.who-we-are-faq.addModal')
                        @include('dashboard.who-we-are-faq.editModal')		
                    </div>	
                </div>
            </div>
            <!-- /Page Wrapper -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '#storeBtn', function (e) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('who-we-are-faq.store') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    dataType: "json",
            ...
            $(document).on('click', '.editBtn', function (e) {
                ...
                $.ajax({
                    type: "get",
                    url: "/admin/who-we-are-faq/edit/" + id,
                    success: function (response) {
                        if (response.status == false) {
                            $('#edit_error_list').html("");
            ...
            $(document).on('click', '.updateBtn', function (e) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{ route('who-we-are-faq.update') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    dataType: "json",
            ...
            $(document).on('click', '.destroyBtn', function (e) {
                ...
                $.ajax({
                    type: "delete",
                    url: "/admin/who-we-are-faq/destroy/" + id,
                    success: function (response) {
                        if (response.status == false) {
                            $('#delete_error_list').html("");
